...
const TRANSLATION_KEYS: Record<string, string> = {
  ...
  "section.main": "navigation.section_main",
  "section.resources": "navigation.section_resources",
  "section.reservations": "navigation.section_reservations",
  "section.operations": "navigation.section_operations",
  "section.admin": "navigation.section_admin",
  "action.back_to_dashboard": "navigation.back_to_dashboard",
};
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        roles: ["admin", "coordinador"],
        children: [
          { href: "/reportes", label: "Dashboard" },
          { href: "/reportes/recursos", label: "Por Recurso" },
          { href: "/reportes/usuarios", label: "Por Usuario" },
          { href: "/reportes/avanzado", label: "Análisis Avanzado" },
          { href: "/reportes/demanda-insatisfecha", label: "Demanda Insatisfecha" },
          { href: "/reportes/cumplimiento", label: "Cumplimiento" },
          { href: "/reportes/conflictos", label: "Conflictos" },
        ],
      },
    ],
  },
...
export function AppSidebar({
  ...
  const isDesignSystem = pathname.includes("/design-system");

  const visibleSections = React.useMemo(() => {
    const sections = navigationSections
      .map((section) => ({
        ...section,
        title: section.title.startsWith("section.")
          ? t(TRANSLATION_KEYS[section.title])
          : section.title,
        items: section.items
          .filter((item) => {
          ...
          .map((item) => ({
            ...item,
            label: TRANSLATION_KEYS[item.href] ? t(TRANSLATION_KEYS[item.href]) : item.label,
            children: item.children?.map((child: { href: string; label: string }) => ({
              ...child,
              label:
    ...
    if (isDesignSystem) {
      const backItem: NavItem = {
        href: "/dashboard",
        label: t("navigation.back_to_dashboard"),
        icon: (
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      return [
        {
          title: t("navigation.design_system"),
          items: [backItem],
        },
        ...sections,
      ];
        ...
        <div key={section.title || sectionIdx} className="space-y-2">
          ...
          <div className="space-y-1">
            {section.items.map((item: NavItem) => {
              ...
              const isBackBtn = isDesignSystem && item.href === "/dashboard";
              const isDisabled = isDesignSystem && !isBackBtn;
              ...
              return (
                <div key={item.href} className="flex flex-col">
                  {hasChildren ? (
                    <button
                      onClick={() => !isDisabled && toggleExpand(item.href)}
                  ...
                  {hasChildren && isExpanded && (
                    <div className="ml-4 mt-1 pl-4 border-l border-[var(--color-border-strong)] space-y-1">
                      {item.children?.map((child: { href: string; label: string }) => {
                        ...
                        return isDisabled ? (
                          <div key={child.href} className={childStyles}>{child.label}</div>
                        ) : (
                          <Link key={child.href} href={child.href} className={childStyles}>{child.label}</Link>
                        );
                      })}
                    </div>
                  )}
