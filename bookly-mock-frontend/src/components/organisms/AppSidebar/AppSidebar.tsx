...
export function AppSidebar({
  ...
  // Determinar si estamos en el sistema de diseño
  const isDesignSystem = pathname.includes("/design-system");

  // Filtrar secciones y sus items según los roles del usuario y traducir etiquetas
  const visibleSections = React.useMemo(() => {
    ...
    // Si estamos en el sistema de diseño, añadir el botón de volver al principio
    if (isDesignSystem) {
        ...
        <div key={section.title || sectionIdx} className="space-y-2">
          ...
          <div className="space-y-1">
            {section.items.map((item: NavItem) => {
              ...
              // En el sistema de diseño, los items normales no son navegables (excepto el de volver)
                    ...
                    <div className="ml-4 mt-1 pl-4 border-l border-[var(--color-border-strong)] space-y-1">
                      {item.children?.map(
                        (child: { href: string; label: string }) => {
                          ...
                          return isDisabled ? (
                            <div key={child.href} className={childStyles}>
                              {child.label}
                            </div>
                          ) : (
                            <Link
