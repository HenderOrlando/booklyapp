...
import { Shield, Smartphone, Key, Copy, CheckCircle2, AlertTriangle } from "lucide-react";
      ...
      <Card className="p-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className={cn(
              "rounded-lg p-2",
              step === "enabled" ? "bg-state-success-100" : "bg-gray-100"
            )}>
              <Shield className={cn(
                "h-6 w-6",
                step === "enabled" ? "text-state-success-600" : "text-gray-400"
              )} />
            </div>
            <div>
        ...
        {step === "disabled" && (
          <div className="mt-4">
            <Alert>
              ...
              <AlertDescription>
                Activar 2FA protege tu cuenta incluso si tu contraseña es comprometida.
              </AlertDescription>
            </Alert>
        ...
        {step === "setup" && (
          <div className="mt-4 space-y-4">
            ...
            <div className="flex items-start gap-6">
              {/* QR Code */}
              <div className="shrink-0 rounded-lg border p-3 bg-white">
                <img src={MOCK_QR_URL} alt="QR Code 2FA" className="h-40 w-40" />
              </div>

              {/* Manual Key */}
              <div className="space-y-3">
                <div>
                  <p className="text-xs font-medium text-[var(--color-text-tertiary)]">Clave manual:</p>
            ...
            <Button variant="ghost" size="sm" onClick={() => setStep("disabled")}>
              Cancelar
            </Button>
          </div>
        )}
          ...
          <div className="mt-4 space-y-4">
            ...
            <div className="rounded-lg border p-4">
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={() => copyToClipboard(MOCK_BACKUP_CODES.join("\n"), "codes")}
                  className="flex items-center gap-1 text-xs text-brand-primary-500 hover:text-brand-primary-600"
                >
                  {copiedCodes ? <CheckCircle2 className="h-3 w-3" /> : <Copy className="h-3 w-3" />}
