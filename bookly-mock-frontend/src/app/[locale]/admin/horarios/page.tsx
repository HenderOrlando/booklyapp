    ...
    <div className="space-y-6 p-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-[var(--color-text-secondary)]">
            Configura las franjas horarias de disponibilidad global para recursos
          </p>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b bg-[var(--color-bg-muted)]">
                <th className="px-4 py-3 text-left font-medium text-[var(--color-text-secondary)]">Día</th>
                <th className="px-4 py-3 text-center font-medium text-[var(--color-text-secondary)]">Estado</th>
                <th className="px-4 py-3 text-center font-medium text-[var(--color-text-secondary)]">Hora inicio</th>
                <th className="px-4 py-3 text-center font-medium text-[var(--color-text-secondary)]">Hora fin</th>
                <th className="px-4 py-3 text-center font-medium text-[var(--color-text-secondary)]">Duración</th>
              </tr>
            </thead>
            <tbody>
              {slots.map((slot) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-center">
                      <button
                        ...
                        className={cn(
                          ...
                          slot.enabled ? "bg-brand-primary-500" : "bg-gray-200"
                        )}
                      >
                        <span className={cn(
                    ...
                    <td className="px-4 py-3 text-center">
                      {slot.enabled ? (
                        <Badge variant={duration >= 12 ? "success" : duration >= 6 ? "primary" : "warning"}>
                          {duration}h
                        </Badge>
                      ) : (
                        <Badge>Cerrado</Badge>
      ...
      <Card className="p-4">
        ...
        <p className="mt-1 text-xs text-[var(--color-text-secondary)]">
          Los días festivos institucionales se sincronizan automáticamente desde el calendario académico.
          Para bloquear fechas específicas, usa la configuración de cada recurso.
        </p>
      </Card>
    </div>
  );
