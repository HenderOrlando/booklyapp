...
test.describe("Resources", () => {
  ...
  test("should display resources list", async ({ page }) => {
    ...
    await expect(page.locator("h1, h2").filter({ hasText: /recursos/i })).toBeVisible();
  });

  test("should navigate to resource detail", async ({ page }) => {
    await page.goto("/es/recursos");
    const firstResource = page.locator("[data-testid='resource-card'], table tbody tr, [class*='card']").first();
  ...
  test("should navigate to create resource page", async ({ page }) => {
    await page.goto("/es/recursos");
    const createBtn = page.locator("a[href*='nuevo'], button:has-text('Crear'), button:has-text('Nuevo')").first();
