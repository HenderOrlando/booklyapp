...
import { 
  AssignCategoryToResourceCommand, 
  AssignCategoriesToResourceCommand 
} from '@apps/resources-service/application/commands/create-resource-category.command';
import { 
...
@Injectable()
@CommandHandler(AssignCategoryToResourceCommand)
export class AssignCategoryToResourceHandler implements ICommandHandler<AssignCategoryToResourceCommand> {
  constructor(
    private readonly resourceCategoryService: ResourceCategoryService,
    private readonly logger: LoggingService,
  ) {}

  async execute(command: AssignCategoryToResourceCommand): Promise<ResourceCategoryResponseDto> {
    try {
      this.logger.log(
        'Executing assign category to resource command',
        `AssignCategoryToResourceHandler - resourceId: ${command.resourceId}, categoryId: ${command.categoryId}`,
        'AssignCategoryToResourceHandler'
      );

      const result = await this.resourceCategoryService.assignCategoryToResource(
        command.resourceId, 
        command.categoryId, 
        command.assignedBy
      );
      ...
      this.logger.error(
        `Failed to assign category to resource: ${error.message}`,
        error.stack,
        'AssignCategoryToResourceHandler'
      );
      throw error;
...
export class AssignCategoriesToResourceHandler implements ICommandHandler<AssignCategoriesToResourceCommand> {
  ...
  async execute(command: AssignCategoriesToResourceCommand): Promise<ResourceCategoryResponseDto[]> {
    try {
      this.logger.log(
        'Executing assign categories to resource command',
        `AssignCategoriesToResourceHandler - resourceId: ${command.resourceId}`,
      ...
      this.logger.error(
        `Failed to assign categories to resource: ${error.message}`,
        error.stack,
        'AssignCategoriesToResourceHandler'
      );
      throw error;
