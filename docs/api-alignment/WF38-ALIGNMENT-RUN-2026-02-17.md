...
**Fecha:** 2026-02-17  
**Scope:** `bookly-frontend` + `bookly-backend`  
...
- Swagger/OpenAPI configurado por servicio:
  - `bookly-backend/apps/auth-service/src/main.ts`
  - `bookly-backend/apps/resources-service/src/main.ts`
  - `bookly-backend/apps/availability-service/src/main.ts`
  - `bookly-backend/apps/stockpile-service/src/main.ts`
  - `bookly-backend/apps/reports-service/src/main.ts`
  - `bookly-backend/apps/api-gateway/src/main.ts`
- AsyncAPI presente por dominio (eventos):
  - `bookly-backend/apps/*/docs/*.asyncapi.yaml`
...
- Contrato FE↔BE validado con test de endpoints:
  - `bookly-frontend/src/__tests__/contracts/endpoint-contract.test.ts`
...
- Mapeo estructurado de errores:
  ...
  - `bookly-frontend/src/infrastructure/http/errorMessageResolver.ts`
- Namespace `errors` cargado en i18n:
  - `bookly-frontend/src/i18n/request.ts`
- Catologo i18n de errores:
  - `bookly-frontend/src/i18n/translations/es/errors.json`
  - `bookly-frontend/src/i18n/translations/en/errors.json`
- Uso en UX:
  - `bookly-frontend/src/app/[locale]/login/page.tsx`
  - `bookly-frontend/src/contexts/AuthContext.tsx`

### 3.3 AuthN/AuthZ + rate limiting

- Guards/decorators de seguridad presentes:
  - `bookly-backend/apps/auth-service/src/infrastructure/guards/*.ts`
  - `bookly-backend/apps/auth-service/src/infrastructure/decorators/*.ts`
- Enforcement en controladores (ejemplo):
  - `bookly-backend/apps/auth-service/src/infrastructure/controllers/users.controller.ts`
- Rate limiting server-side:
  - `bookly-backend/apps/api-gateway/src/application/services/rate-limiter-redis.service.ts`
  - Invocado en proxy:
    - `bookly-backend/apps/api-gateway/src/application/services/proxy.service.ts`
...
- Se alineo config de Playwright al puerto real del FE para evitar falso negativo de webServer:
  - `bookly-frontend/playwright.config.ts`
...
- KPIs operacionales documentados:
  - `bookly-backend/docs/operations/KPIS.md`
- ADRs operacionales/arquitectura:
  - `bookly-backend/docs/adr/ADR-001-auth-service-sot.md`
  - `bookly-backend/docs/adr/ADR-002-event-store-outbox.md`
  - `bookly-backend/docs/adr/ADR-003-dlq-policy.md`
...
- No se encontro runbook formal de rollback/release en `bookly-backend/docs/` para este gate.
...
1. **Alineacion de verificacion de conectividad FE/BE**
   - Archivo: `bookly-frontend/scripts/verify-backend-connectivity.sh`
   - Cambios:
...
2. **Alineacion de Playwright con puerto real del FE**
   - Archivo: `bookly-frontend/playwright.config.ts`
   - Cambios:
